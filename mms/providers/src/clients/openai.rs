...
use crate::stream::{
    ...
    StreamEvent, StreamStart, ToolCallDelta, ToolCallEnd, ToolCallStart, Usage,
};
...
impl OpenAIClient {
    ...
    fn build_request(&self, request: &CompletionRequest) -> ChatRequest {
        ...
        ChatRequest {
            ...
            tools,
            parallel_tool_calls: if request.config.tools.is_empty() {
                None
            } else {
                Some(request.config.parallel_tool_calls)
            },
        ...
        tokio::spawn(async move {
            ...
            while let Some(chunk_result) = byte_stream.next().await {
                let chunk = match chunk_result {
                    ...
                    Err(e) => {
                        let _ = tx
                            .send(Err(StreamError::new("stream_error", e.to_string())))
                            .await;
                        break;
                    }
                };
