...
impl ConfigLoader {
    pub fn new(mms_home: PathBuf) -> Self {
        Self { mms_home }
    }

    pub fn from_default_home() -> AgentResult<Self> {
        let home = find_mms_home()?;
        Ok(Self::new(home))
    }
    ...
    pub fn load_with_overrides(&self, overrides: ConfigOverrides) -> AgentResult<Config> {
        let config_path = self.mms_home.join(CONFIG_FILE_NAME);
        ...
        config.mms_home = self.mms_home.clone();
...
pub fn find_mms_home() -> AgentResult<PathBuf> {
...
pub fn ensure_mms_home() -> AgentResult<PathBuf> {
    let home = find_mms_home()?;
